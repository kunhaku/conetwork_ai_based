...
const DetailPanel: React.FC<DetailPanelProps> = ({ graphData, selectedNode, pipelineStatus }) => {
  ...
  if (!graphData) {
    return (
      <div className="w-96 bg-gray-50 border-l border-gray-200 p-6 hidden lg:block overflow-y-auto text-gray-900">
  ...
  return (
    <div className="w-96 bg-white border-l border-gray-200 flex flex-col h-full shadow-xl z-20 overflow-hidden flex-shrink-0 text-gray-900">
      
      {/* Top: Header */}
      <div className="p-5 border-b border-gray-100 bg-gray-50/50">
        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider">Node Intelligence</h3>
      </div>
          ...
          <div className="animate-fade-in">
               ...
               <div className="flex items-center justify-between mb-2">
                  ...
                  {showSkeleton ? (
                      ...
                      <span className="text-xs font-mono font-bold bg-gray-100 px-2 py-1 rounded text-gray-700">
                       ...
                       <span className="text-[10px] font-bold bg-gray-100 text-gray-400 px-2 py-1 rounded border border-gray-200">
                          WAITING...
                       </span>
                  )}
               </div>
               
               <h2 className="text-2xl font-bold text-gray-900 leading-tight mb-1">{selectedNode.name}</h2>
                    ...
                    {showSkeleton ? (
                        ...
                        <div className="grid grid-cols-2 gap-3 text-xs">
                             <div className="bg-gray-50 p-2 rounded border border-gray-100">
                                 ...
                                 <span className="font-mono font-bold text-gray-800 text-sm">{selectedNode.latestPrice || 'N/A'}</span>
                             </div>
                             <div className="bg-gray-50 p-2 rounded border border-gray-100">
                                 <span className="block text-gray-400 text-[10px]">Market Cap</span>
                                 <span className="font-mono font-bold text-gray-800 text-sm">{selectedNode.marketCap || 'N/A'}</span>
                             </div>
                             <div className="bg-gray-50 p-2 rounded border border-gray-100">
                                 <span className="block text-gray-400 text-[10px]">Revenue</span>
                                 <span className="font-mono font-bold text-gray-800 text-sm">{selectedNode.revenue || 'N/A'}</span>
                             </div>
                             <div className="bg-gray-50 p-2 rounded border border-gray-100">
                                 <span className="block text-gray-400 text-[10px]">Net Income</span>
                                 <span className="font-mono font-bold text-gray-800 text-sm">{selectedNode.netIncome || 'N/A'}</span>
                             </div>
                        </div>
                    )}
                 </div>
            ...
            {showSkeleton ? (
                ...
                (selectedNode.sizeBucket || selectedNode.growthProfile || selectedNode.keyThemes) && (
                    <div className="mb-5 bg-white border border-gray-200 rounded-lg p-3 shadow-sm animate-fade-in relative overflow-hidden">
            ...
            {/* General Note */}
            <div className="mb-5">
                 <p className="text-sm text-gray-700 leading-relaxed">{selectedNode.note}</p>
            </div>

            {/* Risk Factors (Agent F) */}
            {hasQuantitativeData && !hasQualitativeData ? (
                 <div className="mb-5 bg-gray-50 p-3 rounded-lg border border-gray-100 opacity-60">
                ...
                <div className="mb-5 bg-red-50 p-3 rounded-lg border border-red-100">
                     <h4 className="text-[10px] font-bold text-red-400 uppercase mb-2">Risk Analysis...</h4>
                     <div className="space-y-2">
                        <div className="h-3 w-full bg-red-100 rounded animate-pulse"></div>
                        <div className="h-3 w-3/4 bg-red-100 rounded animate-pulse"></div>
                     </div>
                </div>
            ) : selectedNode.riskNotes && (
                <div className="mb-5 bg-red-50 p-3 rounded-lg border border-red-100 animate-fade-in">
                    <h4 className="text-[10px] font-bold text-red-400 uppercase mb-1">Risk Factors</h4>
                    <p className="text-xs text-red-800">{selectedNode.riskNotes}</p>
                </div>
            )}
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2">
                ...
                <span className="bg-gray-200 text-gray-600 text-[10px] px-1.5 py-0.5 rounded-full">{relatedLinks.length}</span>
              </h3>
              
              <div className="space-y-3">
                {relatedLinks.length === 0 ? (
                  <p className="text-sm text-gray-500 italic">No direct connections found.</p>
                ) : (
                  relatedLinks.map((link: any, idx: number) => {
                    ...
                    return (
                      <div key={idx} className={`group p-3 border rounded-lg hover:bg-gray-50 transition-all cursor-default relative
                        ${link.isKeyRelationship ? 'border-amber-200 bg-amber-50/30' : 'border-gray-100'}
                      `}>
                        ...
                        <div className="flex items-center justify-between mb-1 pr-4">
                            <span className="text-sm font-semibold text-gray-800">{counterpart.name}</span>
                        ...
                        <p className="text-xs text-gray-700 border-t border-gray-200/50 pt-2 mt-1">
                            {link.description}
                        </p>
                      </div>
                    );
